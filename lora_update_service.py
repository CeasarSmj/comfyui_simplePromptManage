    ...
    async def update_lora_metadata(
        ...
        try:
            ...
            if model_data.get('images') and len(model_data['images']) > 0:
                for idx, image in enumerate(model_data['images'][:3]):  # 最多下载3张图像
        ...
        for idx, lora_file in enumerate(lora_files):
            try:
                ...
                async def file_progress(percent):
                    if progress_callback:
                        # 计算全局进度
                        global_progress = ((idx / len(lora_files)) * 100) + (percent / len(lora_files))
                        await progress_callback(global_progress)
                ...
                if success:
                    success_count += 1
                    logger.info(f"[{idx+1}/{len(lora_files)}] ✓ {os.path.basename(lora_file)}")
                else:
                    failed_files.append(lora_file)
                    logger.warning(f"[{idx+1}/{len(lora_files)}] ✗ {os.path.basename(lora_file)}: {message}")
